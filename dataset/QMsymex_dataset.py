...
class QMdataset(InMemoryDataset):
    ...
    def __init__(self,
                 ...
                 pre_filter=None):
        self.mode = mode
        super(QMdataset, self).__init__(root, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])

    @property
    def raw_file_names(self):
        return [r'G:\Dataset\QM-symex\sdf']

    @property
    def processed_file_names(self):
        return 'QM_symex_fission_atomref_%s.pt' % self.mode
    ...
    def alchemy_nodes(self, g):
        ...
        for n, d in g.nodes(data=True):
            h_t = []
            # Atom type (One-hot 'H', 'B', 'C', 'O', 'F', 'Cl', 'Br')
            h_t += [
                int(d['a_type'] == x)
                for x in all_atoms
            ]
    ...
    def sdf_graph_reader(self, sdf_file):
        ...
        idx = int(re.compile(r'\d{6}').findall(str(sdf_file))[0])
        ...
        # deduct referential value of U0 U H G
        # l[:, 7:11] -= atomref(node_attr[:, 7].long()).sum(0)
        energy_ref = atomref(node_attr[:, 7].long()).sum(0).reshape(1, -1).float()
        data = Data(
            x=node_attr,
    ...
    def process(self):
        self.target = pd.read_csv(r'G:\Dataset\QM-symex\QM_symex_fission.csv',
                                  index_col=0)
        self.target = self.target[['gap', 'homo', 'lumo', 'mu', 'alpha', 'r2', 'zpve_kcal', 'U0', 'U', 'H', 'G', 'Cv',
                                   'sing_E1', 'trip_E1', 'fission', 'delta']]
        sdf_dir = pathlib.Path(r'G:\Dataset\QM-symex\sdf')
        data_list = []
        for sdf_file in tqdm(sdf_dir.glob("*.sdf")):
            idx = int(re.compile(r'\d{6}').findall(str(sdf_file))[0])
            if self.mode == 'dev' and idx % 10 == 0:
                continue
            elif self.mode == 'valid' and idx % 10 != 0:
                continue
...
if __name__ == '__main__':
    train_dataset = QMdataset(root='data-bin', mode='dev').shuffle()
    valid_dataset = QMdataset(root='data-bin', mode='valid')
